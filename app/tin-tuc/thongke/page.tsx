...
export default function NewsStats() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const res = await fetch("/api/tin-tuc/thongke?t=" + Date.now());
        ...
        toast.error("Lỗi khi tải dữ liệu thống kê tin tức");
      }
    };
  ...
  if (stats.loading) {
    return (
      <div className="flex justify-center items-center min-h-[200px]">
        ...
        <span className="ml-2">Đang tải thống kê tin tức...</span>
      </div>
    );
  }
  ...
  const COLORS = ["#F97316", "#F87171"]; // cam và đỏ
  ...
  return (
    <div className="space-y-6 p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <StatCard title="Tổng số" value={stats.total} icon={<Newspaper />} />
        <StatCard
          title="Đang hiển thị"
          value={stats.visible}
          color="bg-orange-50"
